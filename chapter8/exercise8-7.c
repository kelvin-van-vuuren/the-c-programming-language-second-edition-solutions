...
/* malloc accepts a size request without checking its plausibility; free believes that the block it is asked to free
 * contains a valid size field. Improve these routines so they make more pains with error checking. */

/* Solution:
 * - define max allocation, check if requested bytes greater than in malloc.
 * - in free, check that block size is > 0 and less than max alloc
 */
